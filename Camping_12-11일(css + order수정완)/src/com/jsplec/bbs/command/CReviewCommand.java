...
public class CReviewCommand implements CCommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		// TODO Auto-generated method stub
			...
			// 리뷰를 DB에서 불러오기.
			CReviewDao dao = CReviewDao.getInstance();
...
//			String productNo = request.getParameter("pNo");
			...
			ArrayList<CReviewDto> dtos = dao.reviewList(currentPage, pageSize, productNo);

			// attribute에 dtos라는 이름으로 담아서 보내주자!
			request.setAttribute("CReviewDto", dtos);
			
			// CMainDao/getBoardCount메소드로 전체 글 갯수 구해서 totalRow에 저장
					int totalRow = dao.getBoardCount(productNo);
